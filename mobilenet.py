...
train_generator = train_datagen.flow_from_dataframe(dataframe=train_data, x_col="path", y_col="dx", target_size=(299, 299), batch_size=32, class_mode="categorical")
test_generator = test_datagen.flow_from_dataframe(dataframe=test_data, x_col="path", y_col="dx", target_size=(299, 299), batch_size=32, class_mode="categorical", shuffle=False)

try:
    model = load_model('mobilenet_model.h5', custom_objects={'f1_metric': f1_metric})
...
model_checkpoint = ModelCheckpoint('mobilenet_model.h5', monitor='val_loss', save_best_only=True, verbose=1)
...
# Update the file name for saving metrics
...
with open("mobilenet_metrics.txt", "w") as file:
